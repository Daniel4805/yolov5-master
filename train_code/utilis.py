                ...
                if bndname in label_list:
                    ...
                    for i in range(len(label_list)):
                        if bndname == label_list[i]:
                            cv2.rectangle(image, (int(bndbox_xmin), int(bndbox_ymin)),
                                          (int(bndbox_xmax), int(bndbox_ymax)),
                                          colors_list[i], 2)
...
def yolov3_get_txt(dir, label_list):
    ...
    for filename in tqdm(trainList):
        ...
        for element in roots.findall('object'):
            ...
            # else:
            text_file = str(label_list.index(name)) + ' ' + str(xcenter_percent) + ' ' + str(
                ycenter_percent) + ' ' + str(w_percent) + ' ' + str(h_percent) + '\n'
...
def yolov3_get_train_test_file(dir, val_percent):
    ...
    for filename in images_label_file_List:
        ...
        for i in tqdm(range(len(xmls_List))):
            ...
            image_path = os.path.join(image_label_split_path, filename, "JPEGImages",
                                      xmls_List[i].split(".xml")[0] + ".jpg")
            if i <= val_number:
